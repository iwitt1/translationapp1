...
// ── Prompt version ────────────────────────────────────────────────────────────
export const PROMPT_VERSION = '1.2.0'; // 1.2.0: detect returns confidence; Spanglish fallback to sender's language
...
export function buildMessages({
  ...
  const systemPrompt = `You are a precision translation engine.

Translate the message into ${targetLanguage}.
${modifier}
${contextBlock}
Rules:
- Preserve tone, register, slang, and intent exactly.
- Handle idioms naturally — never translate idioms literally.
...
- All confidence values: floats 0.0–1.0. Use null for undetectable string fields, 0.0 for undetectable confidence.`.trim();
