...
// ── Prompt version ────────────────────────────────────────────────────────────
export const PROMPT_VERSION = '2.1.0'; // 2.1.0: two-way casing fidelity, history-referent resolution, no invented gender forms when unknown (harness findings 2026-07-07). 2.0.0: gpt-5.4 + naturalness-first rewrite.

// ── Model config ──────────────────────────────────────────────────────────────
// Single source of truth for both call sites (api/v1/translate.js + server/index.js).
// TRANSLATE: gpt-5.4 with reasoning effort. 'low' since 2026-07-07: the model
// comparison harness (scripts/model-comparison-test.mjs) showed medium averaged
// 7.5s/call with no quality edge over low except more ambiguity-flag firing;
// low kept every distinctive win (usted register, keigo, neutral-gender,
// idiom energy) at ~2-3s. See decisions.md 2026-07-07.
...
export const TRANSLATE_REASONING_EFFORT = 'low';
...
- Casing and punctuation mirror THIS message exactly, in both directions: if the sender capitalized sentence starts and "I", the translation must be capitalized the same way; if they wrote all-lowercase or skipped final periods, so do you. Casual slang does NOT mean lowercase — judge only by what the sender actually typed, never by the vibe of the conversation. Casual laughter converts to its target-language equivalent ("jajaja" ↔ "lol"/"haha"; Japanese 笑/w; Chinese 哈哈哈); emoji pass through unchanged. Never make a message look more formal, complete, or "correct" than the original.
- Use the conversation history to resolve what the message refers to: reactions, pronouns, and elliptical replies must keep their true referent. A reaction to something ANOTHER person said (e.g. "una vez en la vida!!" teasing the other speaker, "she's going to love it" about someone named earlier) must not be rewritten as if it were about the sender themselves.
...
- When the speaker's gender is unknown and the target language forces gender agreement, prefer natural phrasings that avoid the agreement (e.g. "me emociona mucho" rather than "estoy emocionado/emocionada"). Never invent inclusive spellings ("emocionad@", "emocionade") unless the speaker context explicitly says gender: nonbinary.
