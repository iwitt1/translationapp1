...
// ---------------------------------------------------------------------------
// 3b. Conversation-dedupe policy
// ---------------------------------------------------------------------------

/**
 * Global default conversation-dedupe policy (Phase 3 / migration 017).
 *
 * Decides, per conversation `kind`, whether creating a conversation reuses an
 * existing thread with the same member-set ('dedupe') or always mints a new one
 * ('always_new'). Mirrors the DM_INITIATION override pattern.
 *
 * Resolution order:
 *   1. Check tenants.conversation_policy for the kind.
 *   2. If no tenant override, fall through to DEFAULTS here.
 *
 * NOTE: this is the JS/documentation source of truth. The actual enforcement
 * lives in the SQL RPC public.create_conversation() (migration 017), which encodes
 * the same default and reads the same tenants.conversation_policy jsonb — the
 * dedupe must be atomic + race-safe (via conversations.dedupe_key + a partial
 * unique index), so it cannot run in the app layer. Keep the two in sync; any
 * change here requires a matching change to create_conversation() and a
 * decisions.md entry.
 *
 * Values:
 *   'dedupe'      — reuse the existing thread for this exact member-set (one per set)
 *   'always_new'  — every create mints a fresh conversation
 *
 * Default: direct dedupes (one DM thread per pair), group always-new (groups are
 * first-class objects, not derived from their member set). Confirmed 2026-06-12
 * against cross-app convention (decisions.md / Spec 6).
 */
export const CONVERSATION = {
  ...
  /**
   * Resolve the effective dedupe mode for a conversation kind, merging the tenant
   * override (tenants.conversation_policy jsonb) on top of DEFAULTS.
   *
   * @param {string} kind            — 'direct' | 'group'
   * @param {object} tenantOverride  — parsed jsonb from tenants row (may be {})
   * @returns {'dedupe'|'always_new'}
   */
  resolve(kind, tenantOverride = {}) {
    return tenantOverride[kind] ?? this.DEFAULTS[kind] ?? 'always_new';
  },

  /** True if conversations of this kind should be deduped by member-set. */
  shouldDedupe(kind, tenantOverride = {}) {
    return this.resolve(kind, tenantOverride) === 'dedupe';
  },
};
