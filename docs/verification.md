...
**Last updated:** 2026-07-07 (Added **"Username at onboarding — migration 020"** section — ⏳ gate PENDING on staging. `complete_onboarding()` replaced with a 3-arg version that claims the user-chosen username atomically with activation; display_name denylist added; `change_username()` replaced to allow self-revert to one's own retired handle; onboarding UI gains the field + once-per-year subtext. Deploy order matters: 020 on prod BEFORE the frontend merge. See decisions.md 2026-07-07 ×2. Prior 2026-07-05: Added **"Translate model swap: gpt-5.4 + prompt v2.0.0"** section — ⏳ gate PENDING on staging. Translate calls move to gpt-5.4 medium reasoning with a naturalness-first prompt rewrite after observed literal-translation failures; detect stays gpt-4o-mini. See decisions.md 2026-07-05. Prior 2026-06-23: Added **"Phase 2.1 — Token auth on backend API calls"** section — status **SHIPPED TO PROD via an early/accidental main merge; prod smoke GREEN** (no staging gate ran first; accepted — no users, endpoints only get tighter, graceful network fallback). Negative-path 401 checks still to run via a gate script. Documents the prerequisite (enable Supabase asymmetric JWT signing keys on staging), the curl-based 401/200 checks, the UI smoke (non-null `user_id` in `translation_events`), and a proposed `scripts/api-auth-gate-test.mjs`. See decisions.md 2026-06-23 "Token auth on backend API calls". Prior 2026-06-18: **Phase 3 production cutover EXECUTED on prod** — migrations 016→019 replayed on prod (016 no-op/already-CASCADE, 017 conversations schema, sentinel purge a no-op since messages=0, 018 membership-scoped messages RLS, 019 context_type vocab unify), each verified against its embedded block; `phase3/step1-conversations` merged → `main` (`5251669..c13f8ae`), frontend auto-deployed, broken-sends window closed. **2-user prod smoke GREEN**; 3rd-user/group flows + custom-SMTP deferred (Supabase built-in email caps magic links ~2/hr). See "Phase 3 — Step 4" below + decisions.md/operations.md 2026-06-18. Prior 2026-06-11: Phase 2 Step 7 data-deletion / Right-to-Erasure gate ✅ **PASSED on staging — 37/37 GREEN** (migration 013 `data_deletion_requests` + 6 RPCs + `server/lib/deletion.js` + second Vercel cron `/api/v1/jobs/deletion` 09:00 UTC + `scripts/deletion-gate-test.mjs`). First run was 5/15 — migration 013 hadn't been applied (PostgREST "function not found in schema cache"); applying it fixed all 37. **Load-bearing** assertions confirmed: planted message survives with `sender_id=NULL`; audit row survives `completed` with `user_id` nulled via ON DELETE SET NULL; abuse hash is keyed HMAC not plain SHA-256. Step 7 is the last build step before the prod cutover; **prod replay of 013 pending that cutover.** Same-day prior: Phase 2 Step 6 abandonment gate ✅ **PASSED on staging — 19/19 GREEN**; first run was 18/19 — fixed a dry-run counter bug in `server/lib/abandonment.js` (the `summary.deleted`/`summary.hashed` increments were moved inside the `if (!dryRun)` guards, so a dry run now honestly reports `deleted=0/hashed=0`) and made the gate summary wording unambiguous. No live-sweep behavior change. **Prod replay of 012 pending the Phase 2 cutover.** Prior 2026-06-10: Phase 2 Step 6 abandonment + abuse-monitoring section added — ⏳ **WRITTEN, gate PENDING on staging** (migration 012 + `server/lib/abandonment.js` + Vercel cron + `scripts/abandonment-gate-test.mjs`, statically validated, not yet applied/run). Section documents the 4 assertion phases (dry-run deletes nothing, live sweep deletes+releases+records keyed-HMAC, repeat-record increments, anon EXECUTE denied), the record-then-delete ordering, the cron env/`CRON_SECRET` guard, and run instructions. Step 6 is the last build step before the prod cutover (after Step 7). Earlier same day: Phase 2 Step 5 social-graph + safety gate ✅ **PASSED on staging — 40/40 GREEN** via `scripts/social-graph-gate-test.mjs` after migration 011; the Step 4 discovery gate **re-passed 22/22** confirming the block-filter amend didn't regress discovery. Section documents the 7 assertion phases, the canonical-pair / block-override / atomic-report+block / invite-auto-accept behaviors verified, the discovery-RPC block-filter re-gate, run instructions, and the known max_uses-exhaustion coverage gap. Prod replay of 011 pending the Phase 2 cutover. Earlier same day: Phase 2 Step 4 discovery + username-change gate ✅ **PASSED on staging — 22/22 GREEN** via `scripts/discovery-gate-test.mjs` after migration 010; prod replay pending the Phase 2 cutover. New section documents the 10 assertion categories + the 3 SECURITY DEFINER RPCs + run instructions. Earlier same day: Phase 2 Step 3 RLS adversarial gate ✅ **PASSED on staging — 21/21 GREEN**; Step 4 unblocked. Section documents the harness `scripts/rls-adversarial-test.mjs` + tenant-2/user-C fixture + 7 assertion categories + run instructions. Earlier same day: "Server-side profile inference" gate marked ✅ PASSED on staging; prod enablement still deferred. Prior: Phase 2 Step 2 section — migration 008 schema checks + end-to-end signup→onboard→active flow.)
