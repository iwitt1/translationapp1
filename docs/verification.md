...
**Status: ✅ EXECUTED on prod 2026-06-11 — schema + role live, two checks PENDING.** The coordinated wipe-then-replay cutover ran against the prod project `translationapp1`. Prod previously sat at migration 006 (pre-auth, no RLS) while the already-shipped frontend expected the Phase 2 schema; the cutover migrated the DB to match the live app. See `decisions.md` 2026-06-11 "Phase 2 production cutover executed (prod wipe + replay 007→015)" and `operations.md` "Prod cutover — EXECUTED 2026-06-11".
...
**PENDING (the only items keeping the cutover from fully GREEN):**

- [ ] **Two-user inference path on prod** — exercise the same gate as staging: User A sends a message with a regional/register/gender signal; User B (different `preferred_language`) views it → `POST /api/v1/infer-profile` returns `{"status":"updated",...}`, A's profile row + a `user_profile_events` row land under the `profile_writer` role. (Blocked this session by an email-verification rate limit.)
