...
- **Verified on prod:** single-user smoke PASSED (signup → onboard → `active` + ULP row → message), and the **two-user inference path PASSED live on prod 2026-06-11** — es-AR + casual written to the *sender's* ULP row, two `user_profile_events` rows landed, trust boundary held under `profile_writer`. **Only check still pending: Vercel cron registration on prod** (`/api/v1/jobs/abandonment` 08:00, `/api/v1/jobs/deletion` 09:00).
