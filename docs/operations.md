...
- **Verified on prod:** single-user smoke PASSED (signup → onboard → `active` + ULP row → message). **Still pending next session:** (1) two-user inference path under `profile_writer` (blocked by an email-verification rate limit); (2) Vercel cron registration check (`/api/v1/jobs/abandonment` 08:00, `/api/v1/jobs/deletion` 09:00). See decisions.md 2026-06-11 "Phase 2 production cutover executed" and verification.md "Phase 2 production cutover".
