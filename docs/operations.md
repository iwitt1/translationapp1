...
- `022_realtime_conversation_members.sql` — **(⏳ staging-first pending)** Phase 2.4 conversation-list realtime. **Publication-only**, idempotent DO-block guard (mirrors 004): adds `public.conversation_members` to the `supabase_realtime` publication so the client can subscribe to membership INSERTs (RLS-scoped to the viewer's own rows via `conversation_members_select_member`, 017). No RLS/DDL/data change. Pairs with the `App.jsx` two-channel realtime model. `conversations` deliberately not published (no metadata-change subscriber). Verify: `pg_publication_tables` lists `messages` + `conversation_members`. See decisions.md 2026-07-08 "Conversation-list realtime".
