...
**What's in `/migrations/` today.** Running 000–013 in order against an empty Postgres reproduces the **current staging** schema (010 + 011 gates both GREEN on staging 2026-06-10; 012 gate PASSED on staging 2026-06-11, 19/19 GREEN; 013 written, gate PENDING on staging). **Migrations 007–013 are staging-only so far** — prod has not run them yet (the Phase 2 auth/RLS cutover is a deliberate, coordinated event; see architecture.md §10). ⚠️ *Action: confirm prod's actual migration high-water mark — prod is known to be pre-007, but whether 002–006 ran on prod should be verified before the cutover.*
