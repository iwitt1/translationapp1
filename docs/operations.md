...
**Branch protection (since 2026-07-08, when the repo went public).** `main` is a protected branch — direct pushes are rejected for anyone not on the bypass list (GitHub enforces this via *two* independent mechanisms: classic **Settings → Branches** protection rules **and** newer **Settings → Rules** rulesets — a require-PR setting in *either* blocks a push, so both must permit it). Isaac (repo admin) is on the bypass, so the terminal `git push` to `main` above still works for him; for anyone/anything else (a collaborator or an agent), ship via the **GitHub PR** option instead. Going public unlocked branch protection for free (it was a paid feature on private repos — the Phase 1.5 deferral). See decisions.md 2026-07-08.
...
- `022_realtime_conversation_members.sql` — **(staging + prod — applied 2026-07-08, smoke GREEN)** Phase 2.4 conversation-list realtime. **Publication-only**, idempotent DO-block guard (mirrors 004): adds `public.conversation_members` to the `supabase_realtime` publication so the client can subscribe to membership INSERTs (RLS-scoped to the viewer's own rows via `conversation_members_select_member`, 017). No RLS/DDL/data change. Pairs with the `App.jsx` two-channel realtime model. `conversations` deliberately not published (no metadata-change subscriber). Verify: `pg_publication_tables` lists `messages` + `conversation_members`. See decisions.md 2026-07-08 "Conversation-list realtime".
- `021_settings_screen.sql` — **(staging + prod — applied 2026-07-08, smoke GREEN)** Phase 2.4 account settings screen backing. **Additive + one default flip**, single transaction, idempotent, **no recreate**: adds two `SECURITY DEFINER` RPCs granted `authenticated` — `set_preferred_language(text)` (validates + updates `user_linguistic_profiles.preferred_language`) and `set_display_name(text)` (reuses `complete_onboarding`'s 1–50-char + control-char/bidi denylist, updates `profiles.display_name`); flips `account_settings.discoverable_by_email` **default true→false** (`ALTER COLUMN SET DEFAULT`) + updates the `handle_new_user()` trigger to insert `false` (full body replicated from 007, only that one literal changed); and **backfills existing rows** to `discoverable_by_email=false` (pre-launch, reversible — decisions.md 2026-07-08). No tables/policies touched; `change_username()` (from 020) is reused as-is by the settings screen. Verify with the queries at the foot of the file (RPC signatures + grants, column default = false, backfill count = 0, fresh-signup trigger smoke, both RPC happy/deny paths). See decisions.md 2026-07-08 "Account settings screen".
- `020_onboarding_username.sql` — **(staging + prod — gate GREEN on staging 2026-07-07; replayed on prod 2026-07-07 before the frontend merge)** Username chosen at onboarding: **DROP** the 2-arg `complete_onboarding(text, text)` and **CREATE** the 3-arg version with `p_username text DEFAULT NULL` (drop-then-create, not overload — PostgREST named-arg resolution is ambiguous across overloads; grants reissued, 010 REVOKE/GRANT pattern). When `p_username` is provided the function `PERFORM`s `change_username()` (010) inside the same transaction — the claim and the P1→P3 activation are atomic, so pending accounts can never hold user-chosen handles and the Step 6 abandonment hard-delete + handle-release assumption is preserved unchanged. Also adds a `display_name` control-char/bidi **denylist** check (closes parking-lot "Phase 2 RLS / validation gaps" item 3; denylist not allowlist so international names pass), and **replaces `change_username()`** (same signature — CREATE OR REPLACE, grants survive) to allow **self-revert**: a user may reclaim their own retired handle (row flips retired→active); everyone else stays blocked; 365-day cadence applies to reverts. Old 2-named-arg `rpc()` calls still resolve via the default → deploy-order-safe: DB first, frontend after. Function replaces only — no tables, columns, policies, or data touched. See decisions.md 2026-07-07 "Username chosen at onboarding" + "Self-revert".
