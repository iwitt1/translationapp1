...
**Last updated:** 2026-07-02 (Added "Apply finalized brand (colors, Outfit wordmark, wave-seam logo) to the product frontend" under Product features — the violet/teal/Outfit visual identity finalized this session (decisions.md 2026-07-02) reached only `jistchat-landing.html` so far, not the live app. Prior 2026-06-23 (Marked three items **PROMOTED to roadmap.md Phase 2.2** — "Custom transactional email (SMTP) + sending domain," "Persistent login / stay signed in," and "Sign-out control" — they're committed work now; full design detail kept here. The SMTP item also gained a domain strategy in decisions.md 2026-06-23 "Sending domain now, rebrand later" (neutral holding domain + sending subdomain + domain refs in config not code). See decisions.md 2026-06-23 "Phase 2.1 / 2.2" (auth-first ordering). Prior 2026-06-18 (Phase 3 prod cutover follow-ups: added **"Empty / message-less conversation is visible to the recipient"** under the Phase 3 frontend follow-ups (preferred fix option 1 — hide message-less conversations in the list query; options 2/3 noted) and **"Custom transactional email (SMTP) + sending domain"** under Infrastructure and scale — the production-correct fix for the Supabase built-in ~2/hr magic-link cap that limited the cutover smoke to 2 users, and the unblock for the parked re-prompt/CRM email. Also added (Isaac, 2026-06-18) **"Sign-out control: relocate to a menu + confirm + fix mobile kebab overlap"** and **"Persistent login / stay signed in across refresh & tabs"** under Product features. See decisions.md/operations.md 2026-06-18. Prior 2026-06-12: Added "Tenant-scoped option registry (data-driven vocabularies)" under Platform / infrastructure — Isaac's ask during the Phase 3 frontend build to make enumerated option sets like `context_type`/register editable in one place and per-tenant, instead of the current three-file coupling (DB CHECK + `vocabularies.js` + `translatePrompt.js`). Migration 019 unified the vocab as an interim stop-gap; this registry supersedes the hardcoded CHECK later. See decisions.md 2026-06-12 "Unify context_type vocab". Prior same-day — Added "Conversation threads (multiple topic-chats within one group)" under Product features — parked per Isaac's request during the Phase 3 data-model discussion; recorded the "add nothing now, non-destructive ALTER later" rationale so it's a decision, not a gap. See decisions.md 2026-06-12. Prior 2026-06-11 — Phase 2 Step 7: added "`email_hash_abuse` source split — abandonment vs. voluntary erasure" under Known technical debt — both sweeps share one table/RPC/pepper; the source-column split is parked until something reads the signal for enforcement. Isaac explicitly asked to park it. See decisions.md 2026-06-11. Prior 2026-06-10 — Phase 2 Step 6: annotated "Pending-account re-prompt UX" — the deletion sweep shipped (migration 012 + Vercel cron); the re-prompt **email** half is deliberately parked and decoupled to a future CRM (day-3/day-14 cadence), since no sending domain exists yet. Earlier same day — Phase 2 Step 5: annotated the "Identity, discovery & social graph (deferred)" intro to note the contact-graph / invite-link / blocks / reports **structural prep shipped in migration 011** — the deferred items below are still deferred, only their substrate now exists. Added "Invite max-uses exhaustion gate coverage gap" under Known technical debt — the Step 5 gate can't exercise full max_uses exhaustion with only two tenant-1 users. Earlier same day — Phase 2 Step 4: added "Conversation switcher / inbox IA" under Product features — surfaced scoping the Step 4 app-layer wiring; deferred to the Phase 3 conversation model. Earlier same day: added "Per-tenant reserved-word seed automation" and "Original-case username display" deferrals under Identity, discovery & social graph. Earlier same day: Added then RESOLVED "`nonbinary` gender signal regressed out of the schema in migration 008" under Known technical debt — Isaac chose to restore; migration 009 re-adds it, run on staging, **and replayed on prod 2026-06-11 in the Phase 2 cutover (fully resolved — `ulp_gender_check` includes `nonbinary` on prod)**. Earlier same day: Marked "Move profile inference to server-side" + its sibling "Dialect consistency guard" as BUILT — Option A shipped: `/api/v1/infer-profile` endpoint, raw-pg `FOR UPDATE`, message_id trust boundary, `PROFILE_INFERENCE_ENABLED` on. See decisions.md. Earlier same day: Added Option A build spec under "Move profile inference to server-side" — decided to build directly in Cowork; endpoint + service-role + trust-boundary decision + race fix + steps + estimate. Earlier 2026-06-10 Step 2 review: escalated "Move profile inference to server-side" — client-side path is now fully dead under Phase 2 RLS, flagged its flywheel weight; added "Phase 2 RLS / validation gaps" entry — cache poisoning, realtime RLS, display_name charset. Prior 2026-06-09: added "Identity, discovery & social graph (deferred)" section, consolidated "UI improvements", the Model A email-uniqueness tension, and "Onboarding funnel events".)
...
### Apply finalized brand (colors, Outfit wordmark, logo) to the product frontend
...
- **Why interesting:** Right now there's a visual mismatch between the marketing/case-study page (branded) and the live app (unbranded) — anyone clicking "Try the live demo" lands somewhere that looks like a different product.
- **Why deferred:** Landing-page branding was the immediate need (Phase 2.3); reskinning the live app's Tailwind config and components is a separate, larger pass and wasn't asked for in this round.
- **Trigger:** Before/around the Phase 2.3 public-demo push, so the landing page and the live app read as one product; or whenever polish work on the product UI is prioritized.
