...
| [Phase 2.5 — Group-chat polish (surfaced in 3-user testing)](#phase-25--group-chat-polish-surfaced-in-3-user-testing) | Sender attribution + search-to-add / system messages + group naming | 🔶 Spec 11 + 12 on prod; Spec 13 (naming) built, staging pending |
...
- [~] **Group naming — smart default + user-set title** (High). *(Spec 13, promoted from parking-lot 2026-07-16.)* An unnamed group (incl. a Spec 11-promoted direct→group) shows the other members' names joined ("Ana, Kenji +3") instead of "Group"; a member can rename the group (⋯ menu → `set_conversation_title`, migration 024). **BUILT 2026-07-16** (Cowork): migration 024 + frontend; local `vite build` GREEN. **Run 024 on staging + smoke pending**, then prod-replay before merge. See verification.md "Spec 13".
