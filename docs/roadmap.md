...
>
> **Frontend built 2026-06-12; smoke largely GREEN on Vercel Preview against staging (016–019 applied, both gates 35/35 + 27/27).** `src/App.jsx` rewritten from the single global-room view into a conversation-aware app: `ConversationList` + `ConversationView` + `MessageBubble` + `NewConversationModal` + `InviteModal` (markup ported from `mockups/phase3-conversations.html`), backed by the `src/lib/conversations.js` / `discovery.js` / `translation.js` data layers. **Sends now carry the real `conversation_id`** (the old insert omitted it — the coupling that gated the 017→018 prod replay). **Smoke verified:** dedupe, optimistic send (no dupes), translation + "Original:" tap-to-expand, register + "?" (tooltip clips off-screen — parking-lot), no sub-line on own sends, network-loss retry, live realtime delivery. **Not yet exercised (magic-link rate limit, RPC layer gate-green):** third-user invite/join/leave UI, group rendering. See decisions.md 2026-06-12 "Phase 3 conversation-aware frontend" + verification.md "Phase 3 — Step 3".
