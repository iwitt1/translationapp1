...
- [ ] Token-based authentication on every backend API call, including the chat app's own calls *(still open — now tracked + scoped under Phase 2.1)*
...
**Goal:** Remove the friction that currently blocks onboarding more than ~2 users at a time, so real multi-user testing on prod becomes possible.

- [ ] **Custom transactional email (SMTP) + sending domain.** Replace Supabase's built-in email (magic-link delivery, throttled ~2–4/hr) with a real provider (Resend / Postmark / SES) on a verified sending domain, configured under Supabase → Authentication → SMTP. This is *the* production blocker for onboarding testers — magic-link is the only sign-in path, so the email cap *is* the signup cap (it capped the Phase 3 prod smoke to 2 users). Also unblocks the parked re-prompt / CRM email. **Blocked by Phase 2.1 token auth** — this item widens signup access, so the API endpoints must be locked first. **Domain strategy (no brand yet — decisions.md 2026-06-23 "Sending domain now, rebrand later"):** register a cheap neutral/holding domain now rather than waiting; send from a dedicated **subdomain** (e.g. `mail.<domain>`) so email reputation is isolated from the apex and the web domain; keep all domain references in config (Supabase Auth Site URL + redirect allowlist, Vercel custom domain, provider SMTP creds) **never in code**, so a future rebrand is a settings change; pick a provider that allows multiple verified domains (Resend / Postmark) so a later cutover is add-warm-switch with no code change; on any future migration keep both old + new domains in the redirect allowlist so links already in inboxes don't break. Doing this now is the *cheap* moment — deliverability reputation is the only sticky cost and there's almost none to lose at testing scale. Its own small spec + a DNS/verification (SPF/DKIM/DMARC) step. *(promoted from parking-lot.md "Custom transactional email (SMTP) + sending domain", surfaced 2026-06-18.)*
- [ ] **Persistent login / stay signed in across refresh & tabs.** **Verify-first:** Supabase Auth persists the session in `localStorage` and auto-refreshes tokens by default, so step one is confirming `persistSession` / `autoRefreshToken` in `src/lib/supabase.js` + the Auth session-duration setting *before* scoping any build — it may be largely on already. Removes most re-login friction (and email pressure on the SMTP item above). *(promoted from parking-lot.md "Persistent login / stay signed in across refresh & tabs", surfaced 2026-06-18.)*
- [ ] **Sign-out control — confirm + relocate + fix mobile kebab overlap.** Real bug, not polish: on phone widths the exposed top-right sign-out button overlaps the conversation kebab (⋯), so a mis-tap can force a logout — which, with no persistent session, costs a full magic-link round-trip to recover. **Cheap first step:** a "Sign out?" confirmation prompt. **Proper fix:** move sign-out into an account/overflow menu (also resolves the overlap). *(promoted from parking-lot.md "Sign-out control: relocate to a menu + confirm + fix mobile kebab overlap", surfaced 2026-06-18.)*

### What "Phase 2.2 done" means
- A new tester can be onboarded by magic link without hitting an email rate limit.
- A signed-in user survives a refresh / new tab without re-authenticating.
- Sign-out cannot be triggered by an accidental mobile mis-tap.
