...
| [Phase 2.1 — Close auth / security gaps before widening access](#phase-21--close-auth--security-gaps-before-widening-access) | Lock the API endpoints before widening access | 🔶 Token auth done; 2 Hermes items killed; refresh/rotation gate written, run pending |
...
- [ ] Refresh / rotation behavior verified *(tracked under Phase 2.1 — gate `scripts/auth-refresh-gate-test.mjs` written 2026-07-16, run pending on staging)*
...
- [ ] **Refresh / rotation behavior verified.** *The other still-open Phase 2 "Authentication" item; pairs with persistent login (2.2) — confirm token refresh/rotation behaves correctly once sessions persist.* **Gate WRITTEN 2026-07-16** (`scripts/auth-refresh-gate-test.mjs` — also absorbs the previously-proposed `api-auth-gate-test.mjs` no-token/garbage/valid checks). **Run pending on staging** (needs `.env.rls-test` + a live Preview `STAGING_API_BASE_URL`). Check off after a GREEN run. See verification.md "Phase 2.1 — Refresh / rotation behavior".
...
- **2026-07-16** — Wrap-up of 2.1–2.3: killed the two Hermes-era 2.1 items (stray `translation_events` row + sandbox git-pull gap) as won't-do (→ decisions.md 2026-07-16); wrote the refresh/rotation gate `scripts/auth-refresh-gate-test.mjs` + verification.md section (run pending); wrote the 3+-user share-ready smoke checklist in verification.md (run pending); marked Phase 2.3 own-voice copy rewrite done (landed ~2026-07-15 in a separate session). Phase 2.3 ✅; 2.1/2.2 each down to one run-pending item.
