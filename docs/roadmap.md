...
**Last updated:** 2026-07-16 — Phase 2.1 closed; Phase 2.2 marked done (3+-user smoke ran, findings → 2.5); Phase 2.5 (group-chat polish: Specs 11/12/13) shipped to prod. Full history in [Changelog](#changelog).
**Owner:** Isaac (iwitt1)
...
| [Phase 2.2 — Enable real multi-user testing](#phase-22--enable-real-multi-user-testing) | Remove friction (SMTP, persistent login) for real testers | ✅ Done (2026-07-16 — 3+-user prod test run; findings became Phase 2.5) |
...
- [x] **Share-ready smoke.** With email working, sign up **3+ external accounts** and run a full multi-user smoke on prod (direct + group create, invite/join, cross-language translation, sender names) — the flows the Phase 3 cutover had to defer behind the 2/hr email cap. **RAN 2026-07-16 on prod** (checklist in verification.md "Phase 2.2 — Share-ready multi-user smoke"): the core multi-user flows work; the run **surfaced three group-chat gaps** (sender attribution, add-by-search vs. link-only, groups labeled "Group") → split into **Phase 2.5 (Specs 11/12/13), all shipped to prod 2026-07-16**. Smoke satisfied, follow-ups closed.
...
- [x] **Group naming — smart default + user-set title** (High). *(Spec 13, promoted from parking-lot 2026-07-16.)* An unnamed group (incl. a Spec 11-promoted direct→group) shows the other members' names joined ("Ana, Kenji +3") instead of "Group"; a member can rename the group (⋯ menu → `set_conversation_title`, migration 024), which posts a "X renamed the group to Y" system message (also propagates the rename live). **SHIPPED — on prod 2026-07-16** (Cowork): migration 024 + frontend; staging GREEN → prod smoke GREEN. See verification.md "Spec 13".
...
- Groups read as their members (or a chosen name) rather than a generic "Group".
