...
- [~] **`messages` + `message_translations` RLS: tenant-scoped → membership-scoped (Step 2 / Spec 7).** Ends the "one global room" model at the authorization layer (017 ended it at the schema layer): a user may read/post a message, and read/write its cached translation, only as an active member of its conversation; realtime delivery follows the same predicate. *(migration 018, written 2026-06-12 — policies-only, no DDL/data change; **staging gate `scripts/messages-rls-gate-test.mjs` + prod replay pending; replay AFTER 017**. Sentinel messages go dark + get purged — decisions.md "Retire the global-room sentinel data".)*
