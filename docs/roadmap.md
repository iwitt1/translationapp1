...
- [ ] Scheduled job: re-prompt pending accounts; delete abandoned ones after 30 days, release their system-generated username, record an email **hash** in the abuse-monitoring table *(Step 6 — **written, gate pending on staging.** Vercel cron `/api/v1/jobs/abandonment` (daily 08:00 UTC) + `server/lib/abandonment.js` + migration 012 (two service_role-only `SECURITY DEFINER` helpers). Hard-deletes aged-pending via the admin API; the FK cascade releases the username (no release function by design); records a keyed **HMAC-SHA256** of the email (pepper in env, never in DB, `key_version=1`) before delete. **Re-prompt emails parked → future CRM** (no sending domain yet). Stays unchecked until `scripts/abandonment-gate-test.mjs` exits 0 on staging. See decisions.md 2026-06-10 "Step 6 abandonment + abuse monitoring".)*
