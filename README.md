...
Everything through Phase 3 is shipped to prod and the app is open for external account creation at **`app.jistchat.com`** (magic-link sign-in via Resend). Phase 2 (multi-tenant identity + social graph) cutover GREEN 2026-06-11; Phase 3 (real conversation model) 2026-06-18; Phase 2.1 (token auth + refresh/rotation) and Phase 2.2 (public demo readiness — custom email, persistent login, 3+-user smoke) done 2026-07-16; Phase 2.3 (case-study landing) and Phase 2.4 (settings screen, native-name languages, UI symbology, conversation-list realtime, public repo) done; **Phase 2.5 (group-chat polish) shipped 2026-07-16** — per-sender colored avatars/names, add-to-conversation by search + "X was added" system messages, and group naming. Phase 1.5 (Hermes Agent) was set up but is **paused** — Cowork + Cursor is the working toolchain. **Next up:** Phase 4 (corrections capture). See [`docs/roadmap.md`](docs/roadmap.md) for what's next and [`docs/architecture.md`](docs/architecture.md) §2 for what currently works versus what doesn't.
